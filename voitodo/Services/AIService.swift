...
class AIService {
    ...
    /// Lazily loaded from intent_keywords.json — cached in memory after first load
    private lazy var intentKeywords: [String] = {
        loadStringArray(from: "intent_keywords")
    }()
    ...
    private lazy var productKeywords: [String] = {
        ...
        // Flatten all categories and sort by length descending so multi-word entries match first
    ...
    /// Detects commercial intent and extracts the purchase item using a 2-tier dictionary approach.
    /// Tier 1: intent_keywords.json — checks if the thought has buying intent.
    /// Tier 2: product_keywords.json — scans for known product nouns.
    func detectShoppingIntent(in text: String) -> (isShopping: Bool, query: String?) {
        let textLower = text.lowercased()
        
        // Tier 1: Intent check
        let hasIntent = intentKeywords.contains { textLower.contains($0) }
        ...
        for product in productKeywords {
            ...
            if matched {
                ...
                // Mask matched product to prevent sub-string double-matching
